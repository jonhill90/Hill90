...
See `bash scripts/hostinger.sh --help` for the full command list.
...
# Manual update via the Hostinger API
make dns-sync
```
...
1. **Credentials:** DNS operations require `HOSTINGER_API_KEY` from the encrypted secrets file
...
- **Script:** `scripts/hostinger.sh dns <command>`
